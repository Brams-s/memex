...
impl SearchIndex {
    ...
    fn open_or_create_with_policy(
        ...
        if meta_path.exists() {
            ...
            Ok(Self {
                index,
                fields,
                catalog_path: catalog_path_for_index(dir),
            })
        } else {
    ...
    pub fn add_record(&self, writer: &mut IndexWriter, record: &Record) -> Result<()> {
        ...
        let record_key = if record.record_key.is_empty() {
            record.computed_record_key()
        } else {
            record.record_key.clone()
        };
        doc.add_u64(self.fields.catalog_version, 1);
        writer.delete_term(Term::from_field_text(self.fields.record_key, &record_key));
        doc.add_text(self.fields.record_key, record_key);
    ...
    pub fn get_by_doc_id(&self, doc_id: u64) -> Result<Option<Record>> {
        ...
        Ok(Some(record_from_doc(&self.fields, &doc, catalog.as_ref())?))
    }

    pub fn get_by_record_key(&self, record_key: &str) -> Result<Option<Record>> {
        let reader = self.reader()?;
        let searcher = reader.searcher();
        let term = Term::from_field_text(self.fields.record_key, record_key);
        let query = TermQuery::new(term, IndexRecordOption::Basic);
        let top = searcher.search(&query, &TopDocs::with_limit(1))?;
        let Some((_, addr)) = top.first() else {
            return Ok(None);
    ...
    pub fn doc_ids_by_source_path(&self, source_path: &str) -> Result<Vec<u64>> {
        let reader = self.reader()?;
        let searcher = reader.searcher();
        let term = Term::from_field_text(self.fields.source_path, source_path);
        let query = TermQuery::new(term, IndexRecordOption::Basic);
    ...
    pub fn count_by_source_path(&self, source_path: &str) -> Result<usize> {
        Ok(self.doc_ids_by_source_path(source_path)?.len())
    }
    ...
    pub fn search(&self, options: &QueryOptions) -> Result<Vec<(f32, Record)>> {
        ...
        for (score, addr) in top_docs {
            ...
            results.push((
                score,
                record_from_doc(&self.fields, &doc, catalog.as_ref())?,
            ));
        }
        Ok(results)
    }
        ...
        F: FnMut(Record) -> Result<()>,
    {
        ...
        for segment_reader in searcher.segment_readers() {
            ...
            for doc in store.iter::<TantivyDocument>(segment_reader.alive_bitset()) {
                let doc = doc?;
                let record = record_from_doc(&self.fields, &doc, catalog.as_ref())?;
                f(record)?;
            }
        }
        Ok(())
    ...
    fn catalog_reader(&self) -> Option<crate::catalog::CatalogStore> {
        self.catalog_path
            .exists()
            .then(|| crate::catalog::CatalogStore::open_read_only(&self.catalog_path).ok())
            .flatten()
    }
}
...
fn catalog_path_for_index(index_dir: &Path) -> std::path::PathBuf {
    index_dir
        .parent()
        .unwrap_or(index_dir)
        .join("state")
...
fn build_schema() -> Result<Schema> {
    ...
    builder.add_u64_field("catalog_version", STORED);
    builder.add_text_field("record_key", STRING | STORED);
    ...
    builder.add_text_field("tool_input", TEXT);
    builder.add_text_field("tool_output", TEXT);
    builder.add_text_field("interaction_id", STRING | STORED);
...
fn load_fields(schema: Schema) -> Result<IndexFields> {
    ...
    Ok(IndexFields {
        ...
        interaction_id: get("interaction_id")?,
...
fn record_from_doc(
    fields: &IndexFields,
    doc: &TantivyDocument,
    catalog: Option<&crate::catalog::CatalogStore>,
) -> Result<Record> {
    ...
    let partial = Record {
        source,
        record_key: get_str(fields.record_key).unwrap_or_default(),
        ...
        links: RecordLinks {
            interaction_id: get_str(fields.interaction_id),
    ...
    if let Some(catalog) = catalog
        && let Some(record) = catalog.record_by_key(&partial.record_key)?
    {
        return Ok(record);
    }
    Ok(partial)
...
mod tests {
    ...
    #[test]
    fn record_key_is_computed_stored_and_queryable() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let index = SearchIndex::open_or_create(tmp.path()).expect("open index");
        let original = record(42, "/tmp/session.jsonl");
        let expected_key = original.computed_record_key();
        let mut writer = index.writer().expect("writer");
        index.add_record(&mut writer, &original).expect("add");
        writer.commit().expect("commit");
        drop(writer);
        let rebuilt = record(99, "/moved/session.jsonl");
        let mut writer = index.writer().expect("replacement writer");
        index
            .add_record(&mut writer, &rebuilt)
            .expect("replace by stable key");
        writer.commit().expect("replacement commit");

        let stored = index
            .get_by_record_key(&expected_key)
            .expect("query")
            .expect("record");

        assert_eq!(stored.doc_id, 99);
        assert_eq!(stored.record_key, expected_key);
        assert_eq!(index.doc_count().expect("doc count"), 1);
        assert_eq!(
            index
                .doc_ids_by_source_path("/moved/session.jsonl")
                .expect("source ids"),
            vec![99]
        );
    }
